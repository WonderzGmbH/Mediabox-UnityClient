...
namespace AsyncAwaitUtil {
	public static class IEnumeratorAwaitExtensions {
		...
		public class SimpleCoroutineAwaiter<T> : INotifyCompletion {
			...
			public bool IsCompleted { get { return this._isDone; } }
			...
			public void Complete(T result, Exception e) {
				...
				if (this._continuation != null) {
					RunOnUnityScheduler(this._continuation);
				}
			}

			void INotifyCompletion.OnCompleted(Action continuation) {
				Assert(this._continuation == null);
				Assert(!this._isDone);
		...
		class CoroutineWrapper<T> {
			...
			public CoroutineWrapper(
				...
				SimpleCoroutineAwaiter<T> awaiter) {
				this._processStack = new Stack<IEnumerator>();
				this._processStack.Push(coroutine);
				this._awaiter = awaiter;
			}

			public IEnumerator Run() {
				while (true) {
					var topWorker = this._processStack.Peek();
					...
					try {
						...
						var objectTrace = GenerateObjectTrace(this._processStack);

						if (objectTrace.Any()) {
							this._awaiter.Complete(
								default(T), new Exception(
									GenerateObjectTraceMessage(objectTrace), e));
						} else {
							this._awaiter.Complete(default(T), e);
						}
					...
					if (isDone) {
						this._processStack.Pop();

						if (this._processStack.Count == 0) {
							this._awaiter.Complete((T) topWorker.Current, null);
							yield break;
						}
					}
					...
					if (topWorker.Current is IEnumerator) {
						this._processStack.Push((IEnumerator) topWorker.Current);
					} else {
